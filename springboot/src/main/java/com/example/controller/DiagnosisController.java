...
import com.example.dto.FaceDiagnosisRequestDTO;
...
public class DiagnosisController {
    ...
    @PostMapping("/face")
    public Result<FaceDiagnosisDTO> analyzeFace(@RequestBody FaceDiagnosisRequestDTO request) {
        if (request.getImageBase64() == null || request.getImageBase64().isEmpty()) {
            return Result.error("400", "请上传图片数据");
        }
        try {
            FaceDiagnosisDTO result = diagnosisService.analyzeFace(request.getUserId(), request.getImageBase64());
            return Result.success(result);
        } catch (Exception e) {
            e.printStackTrace();
            return Result.error("500", e.getMessage());
